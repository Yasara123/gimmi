...
<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
      ...
      <script tpye="text/javascript" src="js/chosen.jquery.js"></script>
	...
	$(function(){
	  ...
	  $tabs = $( "#tabs").tabs({
	    tabTemplate: '<li><a href="#{href}">#{label}</a> <span class="ui-icon ui-icon-close" title="Tab schlieÃŸen">Remove Tab</span></li>',
	    add: function( event, ui ) {
	      $( ui.panel ).append( getNewTabContent(tab_counter ));
	  ...
	  $('#tabs li a').live('dblclick', function() {
	    var new_name = prompt("Geben Sie den neuen Namen fÃ¼r den Tab ein","");
	    if(new_name) {
	      new_name = (new_name == "") ? $(this).html() : new_name;
	      $(this).html(new_name);
	    }
	  });
	  
	  //add on click functionality to "new tab" symbol
	  $('#add_tab').live('click', addTab)
	  ...
	  //hin und herschalten zwischen domains hinzufÃ¼gen
	  $('#prev_page').live('click', function() { switchDomain('prev'); })
			 .live('mouseover', function() { $(this).css('cursor', 'pointer'); });
	  $('#next_page').live('click', function() { switchDomain('next'); })
			 .live('mouseover', function() { $(this).css('cursor', 'pointer'); });
	...
	function selectPage(page_id) {
	  var url = 'http://www.' + $(getCurrentTabId('string') + ' .chzn-single span').html();
	  changeiFrameUrl(url);
	}
...
<body>
  ...
  <div id="tabs">
    ...
    <ul id="tab-list">
      <li id="prev_tab">
	<span class="ui-icon ui-icon-triangle-1-w" title="Vorherigen Tab auswählen (Strg + Alt + Pfeiltaste links)">Previous Tab</span>
      </li>
      <li id="next_tab">
	<span class="ui-icon ui-icon-triangle-1-e" title="Nächsten Tab auswählen (Strg + Alt + Pfeiltaste rechts)">Next Tab</span>
      </li>
      <li><a href="#tabs-1">Neuer Tab</a> <span class="ui-icon ui-icon-close" title="Tab schlieÃŸen">Remove Tab</span> </li>
      <li id="add_tab">
	<span class="ui-icon ui-icon-circle-plus" title="Neuen Tab erstellen">Add Tab</span>
      </li>
    </ul>
  ...
  <div id="new_tab" style="display:none;">
    <h2>Auswahlkriterien für zu analysierende Seiten</h2>
    <p>Kriterien können auch leer gelassen werden!</p>
    <form id="new_tab_form_-id-">
      <table style="border:none; width:300px;">
	<tr>
	  <td style="font-weight:bold;">
	    <label for="language_-id-">Sprache</label>
	  </td>
	  <td>
	  <%
	    Language lang = new Language(Database.getInstance());
	    List<String> languages = lang.getAllEntries("name_de", true);
	  ...
	  %>
	  <select name="language_-id-" id="language_-id-" style="width:150px;">
	...
	<tr>
	  <td style="font-weight:bold;">
	    <label for="country_-id-">Land</label>
	  </td>
	  <td>
	    <select name="country_-id-" id="country_-id-" style="width:150px;">
	      <option value="" selected="selected">--</option>
	      <option value="de">Deutschland</option>
	      <option value="at">Österreich</option>
	      <option value="fr">Frankreich</option>
	      <option value="uk">Groß Britannien</option>
	...
	<tr>
	  <td style="font-weight:bold;">
	    <label for="top-category_-id-">Kategorie</label>
	  </td>
	  <td>
	    <select name="top-category_-id-" id="top-category_-id-" style="width:150px;">
	      <option value="" selected="selected">--</option>
	      <option value="1">Gastronomie</option>
	      <option value="2">Kosmetik</option>
	      <option value="3">Nachrichten</option>
	      <option value="4">Sport</option>
	      <option value="5">Hochschulen</option>
	    </select>
	  </td>
	</tr>
	<tr>
	  ...
	  <td>
	    <label for="crawl-earliest_-id-">Von</label> 
	    <input id="crawl-earliest_-id-" type="text" name="crawl-earliest_-id-" size=10 value="dd.mm.yyyy"> 
	    
	    <label for="crawl-latest_-id-">Bis</label> 
	    <input id="crawl-latest_-id-" type="text" name="crawl-latest_-id-" size=10 value="dd.mm.yyyy">
	  </td>
	</tr>
	<tr>
	  <td style="text-align:right;height:40px;vertical-align:bottom;" colspan=2><button id="new_tab_form_submit_-id-">Auswahl erstellen</button></td>
      </table>
    </form>
  </div>
  ...
  <div id="tab_template" style="display:none;">  
	...
	<h2 style="margin:0;">Einschränkungen</h2>
      </div>
	...
	<ul class="ul-horizontal">
	  ...
	  <li><b>Crawlzeitraum: </b> 10.10.2010 - 10.10.2011</li>
	</ul>
	<br/>
	<span><b>50 Seiten</b> zu dieser Auswahl im Korpus</span>
      </div>      
    </div>
    
    <div class="domain-container" style="padding:0;">
      <div class="domain-switch">
	<span id="prev_page" class="ui-icon ui-icon-triangle-1-w" title="Vorhergehende Domain aus Auswahl anzeigen">Previous Page</span>
	<span id="next_page" class="ui-icon ui-icon-triangle-1-e" title="NÃ¤chste Domain aus Auswahl anzeigen">Next Page</span>
      </div>
      <div class="domain">
	<!-- <h2>www.example.com</h2> -->
	<select data-placeholder="Domain auswählen..." class="chzn-select" style="width:350px;" tabindex="4">
	  <option value="0" selected="">example.com</option>
    ...
    <ul class="ul-horizontal">
      <li><b>Sprache: </b>deutsch</li>
      <li><b>Land: </b>Österreich</li>
      <li><b>Kategorie: </b>Sport - Fußball</li>
      <li><b>Crawldatum: </b>08.10.2011</li>
    </ul>
    
    <br/><br/>
    <div class="show-additional-information" style="clear:both;width:200px;">
	<span class="ui-icon ui-icon-circle-plus" style="float:left;">Mehr Informationen zeigen</span>
	<span>Mehr Infos anzeigen</span>
    </div>
